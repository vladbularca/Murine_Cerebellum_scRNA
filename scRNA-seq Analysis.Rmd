...
DimPlot(merged_obj, reduction = "umap", label = TRUE, repel = TRUE) +    NoLegend() + ggtitle("UMAP of Cell Clusters") +  theme(plot.title = element_text(size = 4, face = "bold"))
...
midbrain_genes <- c("Tlx3", "Isl1", "Dlk1", "Otx2", "Sncg")

quantile(merged_obj$MidbrainScore1, probs = 0.99)

sum(merged_obj$MidbrainScore1 > 1.1)

merged_obj <- AddModuleScore(merged_obj, features = list(midbrain_genes), name = "MidbrainScore")

seurat_final <- subset(merged_obj, subset = MidbrainScore1 <= 1.1)
...
```{r}
# subsetting to clusters of interest
